...
public class fconsumo {
    ...
    public DefaultTableModel mostrar(String buscar){
       ...
       sSQL="select c.idconsumo,c.idreserva,c.idproducto,p.nombre,c.cantidad,c.precio_venta "
               ...
               + " where c.idreserva ="+ buscar + " order by c.idconsumo desc";  //parametro busca es el idreserva
    ...
    public boolean Eliminar(vconsumo dts){
        ...
        try {
            PreparedStatement pst = cn.prepareStatement(sSQL);
            pst.setInt(1, dts.getIdconsumo()); // Otener cual es el consumo que se quiere eliminar
            int n= pst.executeUpdate();
            if (n!=0) {
                return true;
            }
